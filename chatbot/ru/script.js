...
function applySuccessfulConnection(displayName) {
    ...
    if (connText) {
        connText.textContent = displayName ? `Подключено к ${displayName}` : 'Подключено';
    }
}
...
async function autoConnectToSameOrigin() {
    ...
    try {
        ...
        // Не удалось подключиться — остаёмся на экране авторизации
        if (connText) connText.textContent = 'Не подключено';
    ...
    fetch(url, {
        ...
        .then(async response => {
            ...
            if (!response.ok) {
                const msg = (data && (data.error_description || data.error || data.reason))
                    || (response.status === 401 ? 'Неверный логин или пароль.' : `Ошибка авторизации (HTTP ${response.status}).`);
        ...
        .then(data => {
            if (data && data.access_token) {
                ...
                const msg = (data && (data.error_description || data.error || data.reason)) || 'Сервер не вернул access_token.';
                showError(msg);
            }
        })
        .catch(err => {
